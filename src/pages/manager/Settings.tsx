...
import { Building2, Bell, Shield, CreditCard, HelpCircle, LogOut, ChevronRight, Moon } from 'lucide-react';
  ...
  const sections = [
    {
      title: 'Organisation',
      items: [
        { icon: <Building2 size={18} />, label: 'Organisation Details', desc: mockOrg.name },
        { icon: <Shield size={18} />, label: 'Manager Permissions', desc: 'Role-based access control' },
      ]
    },
    {
      title: 'Notifications',
      items: [
        { icon: <Bell size={18} />, label: 'Reminder Settings', desc: 'WhatsApp, SMS, In-app' },
      ]
    },
    {
      title: 'Billing',
    ...
    },
    {
      ...
      items: [
        ...
        { icon: <Moon size={18} />, label: 'Appearance', desc: 'Dark mode (default)' },
      ]
    }
  ];
    ...
    <motion.div key="settings" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      ...
      <GlassCard variant="elevated" className="mb-6">
        <div className="flex items-center gap-4">
          <Avatar name={user?.name || 'User'} size="xl" showRing />
          <div>
